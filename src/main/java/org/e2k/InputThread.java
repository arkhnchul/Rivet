...
public class InputThread extends Thread {
    ...
    public void run() {
        ...
        for (; ; ) {
            ...
            if (audioReady && !loadingFile && !gettingAudio) getSample();
            else if (loadingFile) getFileData();
...
//                JOptionPane.showMessageDialog(null, "Error in getFileData()\n" + e.toString(), "Rivet", JOptionPane.ERROR_MESSAGE);
    ...
    private boolean grabWavBlock() {
        ...
        // 16 bit LE
        if ((!audioInputStream.getFormat().isBigEndian()) && (audioInputStream.getFormat().getSampleSizeInBits() == 16)) {
            return grabWavBlock16LE(audioInputStream);
        }
        // 8 bit LE
        else if ((!audioInputStream.getFormat().isBigEndian()) && (audioInputStream.getFormat().getSampleSizeInBits() == 8)) {
